    ...
    <div className="space-y-10">
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex gap-3 items-start shadow-sm">
        ...
        <div>
          ...
          <p className="text-xs text-amber-700 mt-1 font-roboto leading-relaxed">
            When "Auto-release" is enabled, results will be automatically
            emailed to students on the scheduled exam date (or immediately if
            the date has passed). Manually clicking "Release" sends them
            immediately.
          </p>
        </div>
      </div>
      ...
      <section className="space-y-4">
        <h2 className="text-2xl font-bold text-[#17321A] font-montserrat flex items-center gap-2">
          <FileSpreadsheet className="h-6 w-6 text-[#146939]" /> Ready to
          Release
        </h2>
        {pendingExams.length === 0 ? (
          <p className="text-gray-500 font-roboto italic">
            No exams currently pending release.
          </p>
        ) : (
      ...
      {releasedExams.length > 0 && (
        <section className="space-y-4 pt-6 border-t border-gray-100">
          <h2 className="text-xl font-bold text-gray-400 font-montserrat">
            Previously Released
          </h2>
