...
export function SNSLogo({ size = "lg" }: SNSLogoProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center">
      <div
        className={`rounded-full bg-[#00954f]/10 flex items-center justify-center mb-2 ${
          isLarge ? "p-3 sm:p-4" : "p-2"
        }`}
      >
        <ScrollText
          className={`text-[#00954f] ${
            isLarge ? "w-8 h-8 sm:w-10 sm:h-10" : "w-6 h-6"
          }`}
        />
      </div>
