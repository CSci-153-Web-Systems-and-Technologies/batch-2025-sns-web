      ...
      <CardHeader className="border-b border-gray-50 pb-4">
        <div className="flex items-center justify-between">
          ...
          <Link href="/dashboard/exams">
            <Button
              variant="ghost"
              className="text-[#00954f] hover:text-[#17321A] hover:bg-[#e6f4ea] font-montserrat text-xs font-semibold cursor-pointer rounded-xl"
            >
          ...
          {exams.length === 0 ? (
            <div className="text-center py-8">
              ...
              <Link
                ...
                className="text-xs text-[#146939] font-bold hover:underline mt-2 inline-block"
              >
            ...
            exams.map((exam) => (
              <Link key={exam.id} href={`/dashboard/results/${exam.id}`}>
                <div className="flex items-center justify-between p-4 bg-gray-50/50 rounded-2xl border border-gray-100 hover:border-[#00954f]/30 hover:bg-white hover:shadow-md transition-all group cursor-pointer">
                  ...
                  <div
                    className={`text-xs px-3 py-1 rounded-full font-semibold font-roboto border ${
